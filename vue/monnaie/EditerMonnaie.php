...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {  
    ...
    $monnaieControlleur->update($id, $titre, $description, $lien, $photo?? null); // Ajoutez $photoPath ici  
} else {  
    $id = $_GET['id'];  
    $stmt = $monnaieControlleur->show($id);  
    ...
    <section>
        <div class="container">
            <div style="text-align:center">
                <h2>MODIFIER UNE MONNAIE </h2>
                <p>vous pouvez modifier la section Monnaie</p>
            </div>
            <div class="row">
                <div class="column">  
                    <form method="POST" action="#" enctype="multipart/form-data">  
                         <input type="hidden" name="id" value="<?php echo $monnaie['id']; ?>">  
                        <label for="titre">Titre:</label>  
                        <input type="text" id="titre" name="titre" value="<?php echo htmlspecialchars($monnaie['titre']); ?>" required>  
          
                        <label for="description">Description:</label>  
                        <textarea id="description" name="description" style="height:100px"><?php echo htmlspecialchars($monnaie['description']); ?></textarea>  
          
                        <label for="lien">Lien:</label>  
                        <input type="text" id="lien" name="titre" value="<?php echo htmlspecialchars($monnaie['lien']); ?>" required>  
       
                        <label for="photo">Choisir une image:</label>  
                        <input type="file" id="photo" name="photo" accept=".jpg, .jpeg, .png">  
         
                        <button type="submit">Mettre à jour</button>  
                        <a href="AfficherMonnaie.php">Retour à la liste</a>
                    </form>  
                </div>
            </div>
        </div>
