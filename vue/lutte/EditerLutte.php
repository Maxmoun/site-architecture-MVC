...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {  
    ...
    $lutteControlleur->update($id, $titre, $lien, $photo?? null); // Ajoutez $photoPath ici  
} else {  
    $id = $_GET['id'];  
    $stmt = $lutteControlleur->show($id);  
    $lutte = $stmt->fetch(PDO::FETCH_ASSOC);  
}  
?>  
    ...
    <section>
        <div class="container">
            <div style="text-align:center">
                <h2>MODIFIER UNE LUTTE </h2>
                <p>vous pouvez modifier la section lutte</p>
            </div>
            <div class="row">
                <div class="column">
                    <form method="POST" action="#" enctype="multipart/form-data">  
                         <input type="hidden" name="id" value="<?php echo $lutte['id']; ?>">  
                        <label for="titre">Titre:</label>  
                        <input type="text" id="titre" name="titre" value="<?php echo htmlspecialchars($lutte['titre']); ?>" required>  
         
                        <label for="lien">Lien:</label>  
                        <textarea id="lien" name="lien" style="height:100px" required><?php echo htmlspecialchars($lutte['lien']); ?></textarea>  
