...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {  
    ...
    $attaqueControlleur->update($id, $titre, $lien, $photo?? null); // Ajoutez $photoPath ici  
} else {  
    $id = $_GET['id'];  
    $stmt = $attaqueControlleur->show($id);  
    $attaque = $stmt->fetch(PDO::FETCH_ASSOC);  
}  
?>  
            ...
            <div class="row">
                <div class="column">
                    <form method="POST" action="#" enctype="multipart/form-data">  
                         <input type="hidden" name="id" value="<?php echo $attaque['id']; ?>">  
                        <label for="titre">Titre:</label>  
                        <input type="text" id="titre" name="titre" value="<?php echo htmlspecialchars($attaque['titre']); ?>" required>  
         
                        <label for="lien">Lien:</label>  
                        <textarea id="lien" name="lien" style="height:100px" required><?php echo htmlspecialchars($attaque['lien']); ?></textarea>  
          
                        <label for="photo">Choisir une image:</label>  
                        <input type="file" id="photo" name="photo" accept=".jpg, .jpeg, .png">  
        
                        <button type="submit">Mettre à jour</button>  
                         <a href="AfficherAttaque.php">Retour à la liste</a>
                    </form>  
                </div>
            </div>
        </div>
