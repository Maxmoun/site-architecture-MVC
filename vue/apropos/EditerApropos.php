...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {  
    ...
    $aproposControlleur->update($id, $titre, $description, $photo?? null); // Ajoutez $photoPath ici  
} else {  
    $id = $_GET['id'];  
    $stmt = $aproposControlleur->show($id);  
    $apropos = $stmt->fetch(PDO::FETCH_ASSOC);  
}  
?>  
    ...
    <section>
        <div class="container">
            <div style="text-align:center">
                <h2>MODIFIER APROPOS </h2>
                <p>vous pouvez modifier la section apropos</p>
            </div>
            <div class="row">
                <div class="column">
                    <form method="POST" action="#" enctype="multipart/form-data">  
                        <input type="hidden" name="id" value="<?php echo $apropos['id']; ?>">  
                        <label for="titre">Titre:</label>  
                        <input type="text" id="titre" name="titre" value="<?php echo htmlspecialchars($apropos['titre']); ?>" required>  
          
                        <label for="description">Description:</label>  
                        <textarea id="description" name="description" style="height:170px" required><?php echo htmlspecialchars($apropos['description']); ?></textarea>  
         
                        <label for="photo">Choisir une image:</label>  
                        <input type="file" id="photo" name="photo" accept=".jpg, .jpeg, .png">  
         
                        <button type="submit">Mettre à jour</button>  
                        <a href="AfficherApropos.php">Retour à la liste</a>
                    </form>  
                </div>
            </div>
        </div>
